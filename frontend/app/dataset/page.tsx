...
import { PAGE_SIZE, MODEL_TYPE_CLS } from "@/lib/constants";
import { formatName } from "@/lib/utils";
import type { DatasetMeta, DatasetPage } from "@/lib/types";
import { SpinnerIcon, DatabaseIcon, DownloadIcon } from "@/components/ui/icons";
import { CellValue } from "@/components/dataset/CellValue";
...
export default function DatasetPageComponent() {
  ...
  const loadPage = useCallback(async (meta: DatasetMeta, newOffset: number) => {
    setPageLoading(true);
    setPageError(null);
    try {
  ...
  const downloadCsv = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Server error ${res.status}`);
      const data: DatasetPage = await res.json();
      const cols = ["session_id", ...selected.input_columns, ...selected.output_columns];
  ...
  return (
    <div className="h-screen flex flex-col bg-linear-to-br from-indigo-50 to-violet-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <header className="shrink-0 flex items-center justify-between px-5 py-3 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700">
        <h1 className="text-lg font-bold text-slate-900 dark:text-white">Datasets</h1>
        <Link
          href="/"
        ...
        <div className="w-72 shrink-0 border-r border-slate-200 dark:border-slate-700 bg-white/60 dark:bg-slate-900/60 overflow-y-auto flex flex-col gap-2 p-3">
          ...
          {datasets.map((ds) => (
            <button
              ...
              disabled={ds.count === 0}
              title={ds.count === 0 ? "No qualifying records yet" : undefined}
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                ...
                <span
                  className={`text-xs px-1.5 py-0.5 rounded font-semibold shrink-0 ${MODEL_TYPE_CLS[ds.model_type]}`}
                >
                  {ds.model_type}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1 mb-2">
                ...
                <span className="text-xs text-slate-300 dark:text-slate-600 self-center">→</span>
            ...
            <>
              ...
              <div className="shrink-0 flex items-center justify-between px-4 py-2.5 bg-white/80 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-3">
                  ...
                  {page && (
                    <span className="text-xs text-slate-500 dark:text-slate-400 tabular-nums">
                      {rangeStart}–{rangeEnd} of {page.total.toLocaleString()} records
                    </span>
                  )}
                  {pageLoading && <SpinnerIcon className="w-4 h-4 animate-spin text-slate-400" />}
                  {pageError && <span className="text-xs text-red-500">{pageError}</span>}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => loadPage(selected, offset + PAGE_SIZE)}
                    disabled={!page || offset + PAGE_SIZE >= page.total || pageLoading}
                    className="px-3 py-1.5 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  >
                    Next →
                  </button>
              ...
              <div className="flex-1 overflow-auto">
                {page && page.total === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-500 gap-1">
                    ...
                    <p className="text-xs">This dataset has no qualifying data yet</p>
                  </div>
                ) : (
                  <table className="w-full text-xs border-collapse">
                    <thead className="sticky top-0 z-10">
                      <tr>
                        {columns.map((col) => {
                          const isInput = selected.input_columns.includes(col);
                          const isOutput = selected.output_columns.includes(col);
                          return (
                            <th
                              key={col}
