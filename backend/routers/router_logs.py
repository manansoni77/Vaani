...
from logging_module.log_broadcaster import (
    LogBroadcaster,
    levels_at_or_above,
    parse_level,
)
from logging_module.logger import LogEntry, get_engine
...
def list_logs(
    ...
    start_date: datetime | None = Query(
        default=None, description="Window start — defaults to oldest entry"
    ),
    end_date: datetime | None = Query(
        default=None, description="Window end — defaults to newest entry"
    ),
    limit: int = Query(default=10, ge=1, le=1000, description="Max entries to return"),
    offset: int = Query(
        default=0, ge=0, description="Number of entries to skip (for pagination)"
    ),
    order: Literal["newest", "oldest"] = Query(
        default="newest", description="Return newest or oldest first"
    ),
) -> list[LogEntryOut]:
